...
import com.dynamic.NoSuchStateException;

import com.dynamic.model.State;
import com.dynamic.model.impl.StateImpl;
import com.dynamic.model.impl.StateModelImpl;
...
public class StatePersistenceImpl extends BasePersistenceImpl<State>
	...
	public static final FinderPath FINDER_PATH_FETCH_BY_STATENAME = new FinderPath(StateModelImpl.ENTITY_CACHE_ENABLED,
			StateModelImpl.FINDER_CACHE_ENABLED, StateImpl.class,
			FINDER_CLASS_NAME_ENTITY, "fetchBystateName",
			new String[] { String.class.getName() },
			StateModelImpl.STATENAME_COLUMN_BITMASK);
	public static final FinderPath FINDER_PATH_COUNT_BY_STATENAME = new FinderPath(StateModelImpl.ENTITY_CACHE_ENABLED,
			StateModelImpl.FINDER_CACHE_ENABLED, Long.class,
	...
	public State findBystateName(String stateName)
		...
		State state = fetchBystateName(stateName);
	 ...
	 */
	@Override
	public State fetchBystateName(String stateName) throws SystemException {
		return fetchBystateName(stateName, true);
	}
	 ...
	 */
	@Override
	public State fetchBystateName(String stateName, boolean retrieveFromCache)
		...
		if (retrieveFromCache) {
			result = FinderCacheUtil.getResult(FINDER_PATH_FETCH_BY_STATENAME,
					finderArgs, this);
		}
		...
		if (result instanceof State) {
			...
			if (!Validator.equals(stateName, state.getStateName())) {
				result = null;
			}
		}
		...
		if (result == null) {
			...
			if (stateName == null) {
				query.append(_FINDER_COLUMN_STATENAME_STATENAME_1);
			}
			else if (stateName.equals(StringPool.BLANK)) {
				query.append(_FINDER_COLUMN_STATENAME_STATENAME_3);
			}
			else {
				bindStateName = true;

				query.append(_FINDER_COLUMN_STATENAME_STATENAME_2);
			}
			...
			try {
				...
				if (list.isEmpty()) {
					FinderCacheUtil.putResult(FINDER_PATH_FETCH_BY_STATENAME,
						finderArgs, list);
				}
				else {
					if ((list.size() > 1) && _log.isWarnEnabled()) {
						_log.warn(
							"StatePersistenceImpl.fetchBystateName(String, boolean) with parameters (" +
					...
					if ((state.getStateName() == null) ||
							!state.getStateName().equals(stateName)) {
						FinderCacheUtil.putResult(FINDER_PATH_FETCH_BY_STATENAME,
							finderArgs, state);
					}
				}
				...
				FinderCacheUtil.removeResult(FINDER_PATH_FETCH_BY_STATENAME,
					finderArgs);
	 ...
	 */
	@Override
	public State removeBystateName(String stateName)
		throws NoSuchStateException, SystemException {
		State state = findBystateName(stateName);
	 ...
	 */
	@Override
	public int countBystateName(String stateName) throws SystemException {
		FinderPath finderPath = FINDER_PATH_COUNT_BY_STATENAME;
	...
	private static final String _FINDER_COLUMN_STATENAME_STATENAME_1 = "state.stateName IS NULL";
	private static final String _FINDER_COLUMN_STATENAME_STATENAME_2 = "state.stateName = ?";
	private static final String _FINDER_COLUMN_STATENAME_STATENAME_3 = "(state.stateName IS NULL OR state.stateName = '')";
	public static final FinderPath FINDER_PATH_WITH_PAGINATION_FIND_BY_COUNTRYID =
		new FinderPath(StateModelImpl.ENTITY_CACHE_ENABLED,
			StateModelImpl.FINDER_CACHE_ENABLED, StateImpl.class,
	...
	public static final FinderPath FINDER_PATH_WITHOUT_PAGINATION_FIND_BY_COUNTRYID =
		new FinderPath(StateModelImpl.ENTITY_CACHE_ENABLED,
			...
			StateModelImpl.COUNTRYID_COLUMN_BITMASK);
	public static final FinderPath FINDER_PATH_COUNT_BY_COUNTRYID = new FinderPath(StateModelImpl.ENTITY_CACHE_ENABLED,
			StateModelImpl.FINDER_CACHE_ENABLED, Long.class,
	...
	public List<State> findByCountryId(long countryId, int start, int end,
		...
		if (list == null) {
			...
			if (orderByComparator != null) {
			 ...
			 if (pagination) {
				query.append(StateModelImpl.ORDER_BY_JPQL);
			}
	 ...
	 */
	@Override
	public State findByCountryId_First(long countryId,
	 ...
	 */
	@Override
	public State fetchByCountryId_First(long countryId,
		OrderByComparator orderByComparator) throws SystemException {
	 ...
	 */
	@Override
	public State findByCountryId_Last(long countryId,
		OrderByComparator orderByComparator)
		throws NoSuchStateException, SystemException {
	 ...
	 */
	@Override
	public State fetchByCountryId_Last(long countryId,
		OrderByComparator orderByComparator) throws SystemException {
		...
		List<State> list = findByCountryId(countryId, count - 1, count,
				orderByComparator);
	 ...
	 */
	@Override
	public State[] findByCountryId_PrevAndNext(long stateId, long countryId,
		OrderByComparator orderByComparator)
		throws NoSuchStateException, SystemException {
		...
		try {
			...
			array[0] = getByCountryId_PrevAndNext(session, state, countryId,
					orderByComparator, true);

			array[1] = state;

			array[2] = getByCountryId_PrevAndNext(session, state, countryId,
	...
	protected State getByCountryId_PrevAndNext(Session session, State state,
		...
		if (orderByComparator != null) {
			Object[] values = orderByComparator.getOrderByConditionValues(state);
	...
	public void removeByCountryId(long countryId) throws SystemException {
		for (State state : findByCountryId(countryId, QueryUtil.ALL_POS,
	...
	private static final String _FINDER_COLUMN_COUNTRYID_COUNTRYID_2 = "state.countryId = ?";
	...
	public void cacheResult(State state) {
		...
		FinderCacheUtil.putResult(FINDER_PATH_FETCH_BY_STATENAME,
			new Object[] { state.getStateName() }, state);
	...
	protected void cacheUniqueFindersCache(State state) {
		if (state.isNew()) {
			Object[] args = new Object[] { state.getStateName() };

			FinderCacheUtil.putResult(FINDER_PATH_COUNT_BY_STATENAME, args,
				Long.valueOf(1));
			FinderCacheUtil.putResult(FINDER_PATH_FETCH_BY_STATENAME, args,
				state);
		}
		else {
			...
			if ((stateModelImpl.getColumnBitmask() &
					FINDER_PATH_FETCH_BY_STATENAME.getColumnBitmask()) != 0) {
				Object[] args = new Object[] { state.getStateName() };
	...
	protected void clearUniqueFindersCache(State state) {
		...
		FinderCacheUtil.removeResult(FINDER_PATH_COUNT_BY_STATENAME, args);
		FinderCacheUtil.removeResult(FINDER_PATH_FETCH_BY_STATENAME, args);

		if ((stateModelImpl.getColumnBitmask() &
				FINDER_PATH_FETCH_BY_STATENAME.getColumnBitmask()) != 0) {
			args = new Object[] { stateModelImpl.getOriginalStateName() };
	...
	@Override
	public State updateImpl(com.dynamic.model.State state)
		throws SystemException {
		state = toUnwrappedModel(state);
		...
		if (isNew || !StateModelImpl.COLUMN_BITMASK_ENABLED) {
			...
			if ((stateModelImpl.getColumnBitmask() &
				...
				Object[] args = new Object[] {
						stateModelImpl.getOriginalCountryId()
					};
				...
				args = new Object[] { stateModelImpl.getCountryId() };
	...
	protected State toUnwrappedModel(State state) {
		...
		stateImpl.setCountryId(state.getCountryId());
	...
	public void afterPropertiesSet() {
		String[] listenerClassNames = StringUtil.split(GetterUtil.getString(
					com.liferay.util.service.ServiceProps.get(
						"value.object.listener.com.dynamic.model.State")));
